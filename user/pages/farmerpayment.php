...
if (isset($_POST['tabledata'])) {
    ...
    $result = mysqli_query($connection, "SELECT p.*,DATE_FORMAT(p.date,'%d/%m/%Y') AS niceDate, f.name as farmername, f.mobile as farmermobile FROM `farmer_payment` as p                                         
                                        LEFT JOIN farmer as f ON p.farmerid = f.id
                                        WHERE p.status = 1
                                        order by p.date DESC ");
    $data->list = mysqli_fetch_all($result, MYSQLI_ASSOC);

    $result = mysqli_query($connection, "Select id,name from farmer where status='1' order by name");
...
if (isset($_POST['Add'])) {
    ...
    $res = mysqli_query($connection, "INSERT INTO `farmer_payment`(`farmerid`, `date`, `amount`, `mode`, `details`, `updatedby`,`status`)                                           
                                    VALUES('$farmerid','$date','$amount','$type', '$remark','$userid','1')
                                    ");
    if ($res > 0) {
        $msg->value = 1;
...
if (isset($_POST['delete'])) {
    ...
    $res = mysqli_query($connection, "UPDATE `farmer_payment` SET `status`= 0 where id = '$id'");
...
<head>
    ...
    <title><?= $project ?> : Farmer Send Payment</title>
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header bg-green">
                            ...
                            <h4 class="modal-title">Add Farmer Payment Details</h4>
                        </div>
                        ...
                        <div class="modal-footer ">
                            ...
                            <button type="submit" name="Add" value="Add" id='add' class="btn btn-success">Add Farmer Payment</button>
