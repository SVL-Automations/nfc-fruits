...
if (isset($_POST['tabledata'])) {
    ...
    $result = mysqli_query($connection, "SELECT l.*,
                                            (SELECT IFNULL(SUM(p.amount),0) FROM labour_vendor_work as p WHERE p.status = 1 and p.labourvendorid = l.id) as purchaseTotal,
                                            (SELECT IFNULL(SUM(s.amount),0) FROM labour_vendor_payment as s WHERE s.status=1 and s.labourvendorid = l.id ) as sendTotal
                                            FROM `labour_vendor` as l
                                            GROUP BY l.id 
                                        ");
...
if (isset($_POST['Add'])) {
    ...
    $res = mysqli_query($connection, "INSERT INTO `labour_vendor`(`name`, `mobile`,  `details`, `address`, `pending`, `status`)
                                    VALUES('$name','$mobile','$details','$address','0','1')
                                    ");
    if ($res > 0) {
        $msg->value = 1;
        $msg->data = "Labour Vendor Added Successfully";
...
if (isset($_POST['Edit'])) {
    ...
    $updaterain = mysqli_query($connection, "UPDATE `labour_vendor` SET 
    ...
    if ($updaterain > 0) {
        $msg->value = 1;
        $msg->data = "Labour Vendor Update Successfully.";
        $msg->type = "alert alert-success alert-dismissible ";
    } else {
        $msg->value = 0;
...
<head>
    ...
    <title><?= $project ?> : Labour Vendor Details </title>
...
<body class="hold-transition skin-blue sidebar-mini">
    ...
    <div class="wrapper">
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="box box-primary">
                            <div class="box-header with-border">
                                <h3 class="box-title">Labour Vendor Details </h3>
                                <a class="btn btn-social-icon btn-success pull-right" title="Add Labour Vendor" data-toggle="modal" data-target="#modaladdlabourvendor"><i class="fa fa-plus"></i></a>
                            </div>
        ...
        <form id="addlabourvendor" action="" method="post">
            <div class="modal fade" id="modaladdlabourvendor" style="display: none;">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header bg-green">
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">×</span></button>
                            <h4 class="modal-title">Add Labour Vendor</h4>
                        </div>
        ...
        <!-- End Add labour vendor modal -->

        <!-- Edit labour vendor modal -->
        <form id="editlabourvendor" action="" method="post">
            <div class="modal fade" id="modaleditlabourvendor" style="display: none;">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header bg-red">
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">×</span></button>
                            <h4 class="modal-title">Labour Vendor Edit</h4>
                        </div>
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                <label for="exampleInputEmail1">Vensor Name</label>
    ...
    <script>
        $(document).ready(function() {
                    ...
                    success: function(response) {
                        ...
                        $.each(returnedData['list'], function(key, value) {
                            ...
                            button1 = '<button type="submit" name="Edit" id="Edit" ' +
                                ...
                                '" class="btn btn-xs btn-warning edit-button" style= "margin:5px" title=" Edit labour vendor " data-toggle="modal" data-target="#modaleditlabourvendor"><i class="fa fa-edit"></i></button>';
            ...
            //add labour vendor
            $('#addlabourvendor').submit(function(e) {
                ...
                $.ajax({
                    url: $(location).attr('href'),
                    type: 'POST',
                    data: $('#addlabourvendor').serialize(),
                    success: function(response) {
                         console.log(response);
                        var returnedData = JSON.parse(response);
                        // console.log(returnedData);
            ...
            //edit labourvendor 
            $('#editlabourvendor').submit(function(e) {
                $('#editalertclass').removeClass();
                $('#editmsg').empty();
                e.preventDefault();
                ...
                $.ajax({
                    url: $(location).attr('href'),
                    type: 'POST',
                    data: $('#editlabourvendor').serialize(),
                    success: function(response) {
                        // console.log(response);                      
                        var returnedData = JSON.parse(response);
