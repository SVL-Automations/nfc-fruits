...
if (isset($_POST['tabledata'])) {
    ...
    $result = mysqli_query($connection, "SELECT f.*,
                                            (SELECT IFNULL(SUM(p.totalamount),0) FROM farmer_purchase as p WHERE p.status = 1 and p.farmerid = f.id) as purchaseTotal,
                                            (SELECT IFNULL(SUM(s.amount),0) FROM farmer_payment as s WHERE s.status=1 and s.farmerid = f.id ) as sendTotal
                                            FROM `farmer` as f
                                            GROUP BY f.id 
                                        ");
...
if (isset($_POST['Add'])) {
    ...
    $res = mysqli_query($connection, "INSERT INTO `farmer`(`name`, `mobile`,  `details`, `address`, `pending`, `status`,`rate`,`discount`)
                                    VALUES('$name','$mobile','$details','$address','$pending','1','$rate','$discount')
                                    ");
    if ($res > 0) {
        $msg->value = 1;
        $msg->data = "Farmer Added Successfully";
...
if (isset($_POST['Edit'])) {
    ...
    $updaterain = mysqli_query($connection, "UPDATE `farmer` SET 
                                            ...
                                            `rate`='$editrate',`discount`='$editdiscount'
                                            WHERE id = '$id'
                                        ");

    if ($updaterain > 0) {
        $msg->value = 1;
        $msg->data = "Farmer Update Successfully.";
        $msg->type = "alert alert-success alert-dismissible ";
    } else {
        $msg->value = 0;
...
<head>
    ...
    <title><?= $project ?> : Farmer Details </title>
...
<body class="hold-transition skin-blue sidebar-mini">
    ...
    <div class="wrapper">
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="box box-primary">
                            <div class="box-header with-border">
                                ...
                                <a class="btn btn-social-icon btn-success pull-right" title="Add Farmer" data-toggle="modal" data-target="#modaladdfarmer"><i class="fa fa-plus"></i></a>
                            </div>
        ...
        <!-- /.content-wrapper -->
        <!-- Add farmer modal -->
        <form id="addfarmer" action="" method="post">
            <div class="modal fade" id="modaladdfarmer" style="display: none;">
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                <label for="exampleInputEmail1">Farmer Name</label>
                                <input type="text" class="form-control" placeholder="Farmer Name" name="name" id="name" required pattern="[a-zA-Z0-9\s]+">
                            </div>
                        ...
                        <div class="modal-footer ">
                            ...
                            <button type="submit" name="Add" value="Add" id='add' class="btn btn-success">Add Farmer</button>
        ...
        <!-- End Add farmer modal -->

        <!-- Edit farmer modal -->
        <form id="editfarmer" action="" method="post">
            <div class="modal fade" id="modaleditfarmer" style="display: none;">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header bg-red">
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">×</span></button>
                            <h4 class="modal-title">Farmer Edit</h4>
                        </div>
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                <label for="exampleInputEmail1">Farmer Name</label>
                                <input type="text" class="form-control" placeholder="Farmer Name" name="editname" id="editname" required pattern="[a-zA-Z0-9\s]+">
                            </div>
                        ...
                        <div class="modal-footer ">
                            ...
                            <button type="submit" name="Edit" value="Edit" id='Edit' class="btn btn-success">Edit Farmer</button>
                            <button type="button" class="btn pull-right btn-warning" data-dismiss="modal">Close</button>
                        </div>
                    </div>
                    <!-- /.modal-content -->
                    ...
                    success: function(response) {
                        ...
                        $.each(returnedData['list'], function(key, value) {
                            ...
                            button1 = '<button type="submit" name="Edit" id="Edit" ' +
                                ...
                                '" data-mobile="' + value.mobile + '" data-rate="' + value.rate +
                                '" data-discount="' + value.discount + 
                                '" data-address="' + value.address + '" data-details="' + value.details +
                                '" data-pending="' + value.pending +
                                '" class="btn btn-xs btn-warning edit-button" style= "margin:5px" title=" Edit Farmer " data-toggle="modal" data-target="#modaleditfarmer"><i class="fa fa-edit"></i></button>';
